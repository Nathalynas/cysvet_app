...
final _animalHistoryProvider = FutureProvider.autoDispose
    .family<_AnimalHistoryData, AnimalSummaryModel>((ref, animal) async {
      ...
      final events = [...historyPayload.events]
...
class _AnimalHistoryMetricGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = [
      _HistoryInfoItem(label: 'Touro IA', value: _valueOrDash(animal.touroIa)),
      _HistoryInfoItem(
        label: 'Lactações',
...
List<_HistoryDetail> _calculatedResultItems({
  ...
  final lastBirth =
      _dateFromEntry(latestEntry, 'dataUltimoParto') ?? animal.dataUltimoParto;
  final dryOffForecast = _dateFromEntry(latestEntry, 'previsaoSecagem');
  final calvingForecast =
      _dateFromEntry(latestEntry, 'previsaoParto') ??
      _dateFromEntry(latestEntry, 'dataPrevistaParto');
  final prePartumDate =
      _dateFromEntry(latestEntry, 'dataPreParto') ??
      _dateFromEntry(latestEntry, 'entradaPreParto');

  final intervals = _iaIntervalsFromEntry(latestEntry);
  final resolvedIntervals = intervals.isNotEmpty
      ? intervals
      : iaHistory.intervals;
  ...
  return [
    ...
    _HistoryDetail(
      ...
      value: _formatDoubleMonths(
        _doubleFromEntry(latestEntry, 'idadePrimeiraIa'),
      ),
    ),
    _HistoryDetail(
      label: 'Mês do parto',
    ...
    _HistoryDetail(
      ...
      value: _formatDoubleDays(
        _doubleFromEntry(latestEntry, 'mediaIntervaloIa') ??
            iaHistory.averageInterval,
      ),
    ),
    _HistoryDetail(
    ...
    _HistoryDetail(
      ...
      value: _valueOrDash(
        _stringFromEntry(latestEntry, 'mesSecagem') ??
            _monthLabel(dryOffForecast),
      ),
    ),
    _HistoryDetail(
    ...
    _HistoryDetail(label: 'Data pré-parto', value: formatDate(prePartumDate)),
    _HistoryDetail(
      label: 'Mês pré-parto',
      value: _valueOrDash(
        _stringFromEntry(latestEntry, 'mesPreParto') ??
            _monthLabel(prePartumDate),
      ),
    ),
    _HistoryDetail(
...
class _AnimalMovementHistoryPanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (changes.isEmpty) {
      return const _HistoryEmptyPanel(
        message:
            'Nenhuma alteração ou movimentação registrada para este animal.',
      );
    }
